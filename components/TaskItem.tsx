...
export function TaskItem({ task, onEdit, onDelete, onToggleStatus }: TaskItemProps) {
  ...
  // Check if task has a due date and if it's approaching
  const hasReminder = task.dueDate && task.status !== "completed"
  ...
  const isDueSoon =
    dueDate &&
    !isOverdue &&
    dueDate.getTime() - now.getTime() < 24 * 60 * 60 * 1000 && // Less than 24 hours
      ...
      >
        ...
        {hasReminder && (
          <View className="absolute top-3 right-3">
            {isOverdue ? (
              <View className="bg-red-100 dark:bg-red-900/30 p-1 rounded-full">
                <Ionicons name="alert-circle" size={16} color="#EF4444" />
              </View>
            ) : isDueSoon ? (
              <View className="bg-amber-100 dark:bg-amber-900/30 p-1 rounded-full">
                <Ionicons name="time" size={16} color="#F59E0B" />
              </View>
            ) : (
              <View className="bg-blue-100 dark:bg-blue-900/30 p-1 rounded-full">
        ...
        <View className="flex-row justify-between items-start mb-4">
          <View className="flex-row items-center flex-1 pr-4">
            <Checkbox checked={task.status === "completed"} onChange={() => onToggleStatus(task.id)} size={24} />
            <View className="flex-1 ml-4">
              ...
              {task.description ? (
                <Text className="mt-1 text-sm text-gray-600 dark:text-gray-400 leading-5">{task.description}</Text>
              ) : null}
            </View>
          </View>

          <View className="flex-row gap-2">
            <TouchableOpacity onPress={() => onEdit(task)} className="bg-blue-50 dark:bg-blue-900/30 p-2 rounded-xl">
              <Ionicons name="pencil" size={16} color="#3B82F6" />
            </TouchableOpacity>
            <TouchableOpacity onPress={handleDelete} className="bg-red-50 dark:bg-red-900/30 p-2 rounded-xl">
        ...
        {totalSubtasks > 0 && (
          <View className="mb-4">
            <View className="flex-row justify-between items-center mb-2">
              <Text className="text-sm font-medium text-gray-700 dark:text-gray-300">Subtasks Progress</Text>
              <Text className="text-sm text-gray-500 dark:text-gray-400">{subtasksProgress}</Text>
            </View>
        ...
        <View className="flex-row flex-wrap gap-2 mb-4">
          ...
          >
            <Ionicons
              ...
              style={{ marginRight: 4 }}
            />
            <Text className={`text-xs font-medium ${statusConfig.textColor}`}>{getStatusLabel(task.status)}</Text>
          </View>

          {task.dueDate && (
            <View
              className={`
              border px-3 py-1 rounded-full flex-row items-center
              ${
                isOverdue
          ...
          <View className="flex-row flex-wrap gap-2 mb-4">
            {task.tags.map((tag) => (
              <View key={tag} className="bg-blue-100 dark:bg-blue-900/30 px-2 py-1 rounded-lg">
                <Text className="text-xs font-medium text-blue-700 dark:text-blue-300">#{tag}</Text>
              </View>
            ))}
          </View>
        )}
